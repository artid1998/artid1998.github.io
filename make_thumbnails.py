    ...
    for i, img_path in enumerate(image_files):
        try:
            with Image.open(img_path) as img:
                ...
                save_path = folder / f"{img_path.stem}.jpg"
                img_main.save(save_path, "JPEG", quality=90)
                ...
                print(f"       원본: {original_size[0]}x{original_size[1]} → 메인: {img_main.size[0]}x{img_main.size[1]} / 썸네일: {img_thumb.size[0]}x{img_thumb.size[1]}")

                # 첫 번째 이미지를 thumb.jpg로 저장
                if i == 0:
                    thumb_main_path = thumb_dir / "thumb.jpg"
                    img_thumb.save(thumb_main_path, "JPEG", quality=85)
                    print(f"       → thumb.jpg 생성 완료")

        except Exception as e:
            print(f"[오류] {img_path.name}: {e}")

    print()
    print(f"완료! thumbnails/ 폴더에 저장되었습니다.")
    print("=" * 60)

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("사용법: python make_thumbnails.py \"img/slides/프로젝트폴더명\"")
        print("예시:   python make_thumbnails.py \"img/slides/한국메인라인\"")
    else:
